...
lazy val scalaJsDom = libraryDependencies += ("org.scala-js" %%% "scalajs-dom" % scalaJSdomVersion).cross(CrossVersion.for3Use2_13)
...
lazy val jsext = libraryDependencies += ("org.querki" %%% "querki-jsext" % jsextVersion).cross(CrossVersion.for3Use2_13)
  //libraryDependencies += "org.querki" %%% "querki-jsext" % jsextVersion
...
//  scalaJsDom,
//  jsext,
//  Compile / npmDeps += Dep("ace-diff", aceDiffVersion, List("ace-diff.min.js", "ace-diff.min.css"), true)
//)
//
...
//  scalatags,
//  jsext,
//  Compile / npmDeps += Dep("bootstrap-slider", bootstrapSliderVersion, List("bootstrap-slider.min.js", "bootstrap-slider.min.css"))
//)
//
lazy val bootstrapnative = project.in(file("bootstrapnative")) enablePlugins (ScalaJSBundlerPlugin) settings(
  scalaJsDom,
  libraryDependencies += ("org.openmole" %%% "sortable-js-facade" % scalaJSortableVersion).cross(CrossVersion.for3Use2_13),
  laminar,
  Compile / npmDependencies += "bootstrap.native" -> bootstrapNativeVersion,
  //Compile / npmDependencies += "sortablejs" -> sortableVersion
) dependsOn (tools)
//
//
